...
public:
  struct LinkedNode {
    int value{0};
    shared_ptr<LinkedNode> next = nullptr;
    shared_ptr<LinkedNode> prev = nullptr;
    LinkedNode(int val) : value{val} {}
  };
  ...
  bool enQueue(int value) {
    ...
    auto node = make_shared<LinkedNode>(value);
    if (size == 0) {
      head = node;
      tail = head;
    } else {
...
private:
  int size{0};
  int maxSize{0};
  shared_ptr<LinkedNode> head = nullptr;
  shared_ptr<LinkedNode> tail = nullptr;
};
