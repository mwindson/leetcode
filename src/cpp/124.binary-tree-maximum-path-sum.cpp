...
public:
  ...
  int maxFromNode(TreeNode *node) {
    if (node == nullptr) {
      return 0;
    }
    int num = node->val;
    max_sum = max(max_sum, num);
    int left_max = maxFromNode(node->left);
    int right_max = maxFromNode(node->right);
    max_sum =
        max(max_sum, max(max(left_max, right_max), left_max + right_max) + num);
